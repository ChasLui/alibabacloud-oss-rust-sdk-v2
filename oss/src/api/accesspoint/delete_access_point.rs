...
pub struct DeleteAccessPointResult {
...
impl Client {
    ...
    /// # Arguments
    ///
    /// * `request` - The `DeleteAccessPointRequest` containing the bucket name
    ///   and the access point name.
    ///
    /// # Examples
    ///
    /// ```no_run
    /// # use alibabacloud_oss_sdk_rust_v2::api::accesspoint::DeleteAccessPointRequest;
    /// # use alibabacloud_oss_sdk_rust_v2::client::Client;
    /// # use alibabacloud_oss_sdk_rust_v2::config::Config;
    /// #
    /// # tokio_test::block_on(async {
    /// let client = Client::new(&Config::default());
    /// let request = DeleteAccessPointRequest {
    ///     bucket: "my-bucket".to_string(),
    ///     access_point_name: Some("my-ap".to_string()),
    ///     ..Default::default()
    /// };
    ///
    /// match client.delete_access_point(&request).await {
    ///     Ok(result) => {
    ///         println!("Access point deleted: {:?}", result.common.status);
    ///     }
    ///     Err(error) => {
    ///         eprintln!("Failed to delete access point: {}", error);
    ///     }
    /// }
    /// # })
    /// ```
    pub async fn delete_access_point(
    ...
    ) -> Result<DeleteAccessPointResult, Box<dyn std::error::Error + Send + Sync>> {
        let mut input = OperationInput {
            op_name: "DeleteAccessPoint".to_string(),
        ...
        let mut result = DeleteAccessPointResult::default();
