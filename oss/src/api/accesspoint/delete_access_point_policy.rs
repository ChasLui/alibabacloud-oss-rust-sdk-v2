...
pub struct DeleteAccessPointPolicyResult {
...
impl Client {
    ...
    /// # Arguments
    ///
    /// * `request` - The `DeleteAccessPointPolicyRequest` containing the bucket
    ///   name and the access point name.
    ///
    /// # Examples
    ///
    /// ```no_run
    /// # use alibabacloud_oss_sdk_rust_v2::api::accesspoint::DeleteAccessPointPolicyRequest;
    /// # use alibabacloud_oss_sdk_rust_v2::client::Client;
    /// # use alibabacloud_oss_sdk_rust_v2::config::Config;
    /// #
    /// # tokio_test::block_on(async {
    /// let client = Client::new(&Config::default());
    /// let request = DeleteAccessPointPolicyRequest {
    ///     bucket: "my-bucket".to_string(),
    ///     access_point_name: Some("my-ap".to_string()),
    ///     ..Default::default()
    /// };
    ///
    /// match client.delete_access_point_policy(&request).await {
    ///     Ok(result) => {
    ///         println!("Access point policy deleted: {:?}", result.common.status);
    ///     }
    ///     Err(error) => {
    ///         eprintln!("Failed to delete access point policy: {}", error);
    ///     }
    /// }
    /// # })
    /// ```
    pub async fn delete_access_point_policy(
    ...
    ) -> Result<DeleteAccessPointPolicyResult, Box<dyn std::error::Error + Send + Sync>> {
        let mut input = OperationInput {
            op_name: "DeleteAccessPointPolicy".to_string(),
        ...
        let mut result = DeleteAccessPointPolicyResult::default();
...
mod tests {
    ...
    #[tokio::test]
    #[serial_test::serial]
    async fn test_delete_access_point_policy() {
        ...
        let result = client
            .delete_access_point_policy(&DeleteAccessPointPolicyRequest {
                bucket: config.bucket.clone(),
                access_point_name: Some(ap_name.clone()),
                ..Default::default()
            })
        ...
        assert!(
            result.is_ok(),
            "delete_access_point_policy failed: {:?}",
            result.err()
        );
