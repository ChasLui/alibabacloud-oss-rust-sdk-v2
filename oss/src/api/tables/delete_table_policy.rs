...
pub struct DeleteTablePolicyResult {
...
impl Client {
    ...
    pub async fn delete_table_policy(
        &self,
        request: &DeleteTablePolicyRequest,
    ) -> Result<DeleteTablePolicyResult, Box<dyn std::error::Error + Send + Sync>> {
        let mut input = OperationInput {
            op_name: "DeleteTablePolicy".to_string(),
        ...
        let mut result = DeleteTablePolicyResult::default();
...
mod tests {
    ...
    #[test]
    fn test_delete_table_policy_request_carries_no_parameters() {
        let request = DeleteTablePolicyRequest {
