...
/// Deletes the encryption rules of a table bucket.
...
pub struct DeleteTableBucketEncryptionResult {
...
impl Client {
    ...
    pub async fn delete_table_bucket_encryption(
        &self,
        request: &DeleteTableBucketEncryptionRequest,
    ) -> Result<DeleteTableBucketEncryptionResult, Box<dyn std::error::Error + Send + Sync>> {
        let mut input = OperationInput {
            op_name: "DeleteTableBucketEncryption".to_string(),
        ...
        let mut result = DeleteTableBucketEncryptionResult::default();
...
mod tests {
    ...
    #[test]
    fn test_delete_table_bucket_encryption_request_carries_no_parameters() {
        let request = DeleteTableBucketEncryptionRequest {
