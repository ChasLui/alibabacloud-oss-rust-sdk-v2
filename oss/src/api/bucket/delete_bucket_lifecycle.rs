...
pub struct DeleteBucketLifecycleResult {
...
impl Client {
    ...
    /// # Arguments
    ///
    /// * `request` - The `DeleteBucketLifecycleRequest` containing the bucket name.
    ///
    /// # Examples
    ///
    /// ```no_run
    /// # use alibabacloud_oss_sdk_rust_v2::api::bucket::DeleteBucketLifecycleRequest;
    /// # use alibabacloud_oss_sdk_rust_v2::client::Client;
    /// # use alibabacloud_oss_sdk_rust_v2::config::Config;
    /// #
    /// # tokio_test::block_on(async {
    /// let client = Client::new(&Config::default());
    /// let request = DeleteBucketLifecycleRequest {
    ///     bucket: "my-bucket".to_string(),
    ///     ..Default::default()
    /// };
    ///
    /// match client.delete_bucket_lifecycle(&request).await {
    ///     Ok(result) => {
    ///         println!("Bucket lifecycle deleted: {:?}", result.common.status);
    ///     }
    ///     Err(error) => {
    ///         eprintln!("Failed to delete bucket lifecycle: {}", error);
    ///     }
    /// }
    /// # })
    /// ```
    pub async fn delete_bucket_lifecycle(
    ...
    ) -> Result<DeleteBucketLifecycleResult, Box<dyn std::error::Error + Send + Sync>> {
        let mut input = OperationInput {
            op_name: "DeleteBucketLifecycle".to_string(),
        ...
        let mut result = DeleteBucketLifecycleResult::default();
...
mod tests {
    ...
    async fn test_delete_bucket_lifecycle() {
        ...
        let bucket_name = generate_unique_bucket_name("delete-bucket-lifecycle");
        ...
        let result = client
            ...
            .await;
        assert!(result.is_ok(), "delete_bucket_lifecycle failed: {:?}", result.err());
